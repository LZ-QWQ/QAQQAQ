...
db_root=E:/Code/QAQQAQ/wavenet/BZNSYP/Wave
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    winpty py $VOC_DIR/mksubset.py $db_root $data_root \
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for s in ${datasets[@]};
    do
      winpty py $VOC_DIR/preprocess.py wavallin $data_root/$s ${dump_org_dir}/$s \
    ...
    winpty py $VOC_DIR/compute-meanvar-stats.py train_list.txt $dump_org_dir/meanvar.joblib
    rm -f train_list.txt

    # Apply normalization
    for s in ${datasets[@]};
    do
      winpty py $VOC_DIR/preprocess_normalize.py ${dump_org_dir}/$s $dump_norm_dir/$s \
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    winpty py $VOC_DIR/train.py --dump-root $dump_norm_dir --preset $hparams \
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    do
      ...
      winpty py $VOC_DIR/evaluate.py $dump_norm_dir/$s $eval_checkpoint $dst_dir \
