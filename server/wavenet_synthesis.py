...
import wavenet_emmm.audio as audio
from wavenet_emmm.hparams_wavenet import hparams

from wavenet_emmm.train import to_categorical

from nnmnkwii.datasets import FileSourceDataset, FileDataSource
from torch.utils import data as data_utils
from wavenet_emmm.evaluate import dummy_collate, to_int16
from torch.nn import functional as F
from scipy.io import wavfile

def batch_wavegen(device, model, c=None, g=None, fast=True, tqdm=tqdm):
    from wavenet_emmm.train import sanity_check
...
def wav_syn(checkpoint_path,output_dir,mel_filename,force_cpu=True):
    torch.set_num_threads(4)
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")

    dst_dir = output_dir
    conditional_path = mel_filename
    ...
    output_html = None
    speaker_id = None
    preset = os.path.join('wave_conf','biaobei.json')
    ...
    from wavenet_emmm.train import build_model
    ...
    for idx, (x, y, c, g, input_lengths) in enumerate(data_loader):
        ...
        # DO generate
        y_hats = batch_wavegen(device,model, c=c, g=g, fast=True, tqdm=tqdm)
