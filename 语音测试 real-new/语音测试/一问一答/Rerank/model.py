...
class SiameseQACNN(object):
    def __init__(self, config):
        ...
        self.logits = self._build(self.embeddings)
        self.res = self._build(self.embeddings)
        # 损失和精确度
    ...
    def _build(self, embeddings):
        ...
        with tf.variable_scope('fc',reuse=tf.AUTO_REUSE) as scope:
            ...
            logits = self.fc_layer(con, 3, 'fc_layer')
    ...
    def _add_loss_op(self, logits):
        ...
        loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits,labels=tf.cast(tf.reshape(self._y, [2,-1,3]),dtype=tf.float32))
